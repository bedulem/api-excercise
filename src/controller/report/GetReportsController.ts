...
export class GetReportsController extends BaseHttpController {
    ...
    public async index(@request() request: Request, @response() response: Response): Promise<Response>{
        const report: Report[] = await this.reportRepository.findAllReports(request.query.userId?.toString(), Number(request.query.dateFrom), Number(request.query.dateTo))
